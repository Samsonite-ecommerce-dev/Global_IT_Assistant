...
namespace Samsonite.Library.Web.Business.Basic
{
    public class UsersService : IUsersService
    {
        ...
        public UsersService(IBaseService baseService, IEncryptService encryptService, appEntities appEntities, logEntities logEntities)
        {
        ...
        public QueryResponse<UserInfo> GetQuery(UsersSearchRequest request)
        {
            ...
            if (request.UserLoginType > 0)
            {
                _list = _list.Where(p => p.LoginType == request.UserLoginType);
            }
        ...
        public PostResponse Add(UsersAddRequest request)
        {
            ...
            using (var Trans = _appDB.Database.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        if (!CheckHelper.CheckEmail(request.UserEmail))
                        {
                            throw new Exception(_languagePack["users_edit_message_email_error"]);
                        }

                        UserInfo objUserInfo = _appDB.UserInfo.Where(p => p.Email == request.UserEmail).SingleOrDefault();
                        if (objUserInfo != null)
                        {
                    ...
                    string _privateKey = _encryptService.CreatePrivateKey(12);
                    string _encryptPassword = _encryptService.EncryptPassword(request.Password, _privateKey);
                    UserInfo objData = new UserInfo()
                    {
                        ...
                        Remark = request.Memo,
                        LoginType = request.UserLoginType,
                    ...
                    Trans.Commit();
                    //添加密码日志
                    _baseService.PasswordLog(new WebAppPasswordLog()
                    {
                        Account = request.UserName,
                        Password = _encryptPassword,
                        UserID = objData.Userid,
                        IP = _baseService.CurrentRequestIP,
                        Remark = string.Empty,
                        AddTime = DateTime.Now
                    });
                    //添加日志
                    _baseService.InsertLog<UserInfo>(objData, objData.Userid.ToString());
                    //返回信息
            ...
            using (var Trans = _appDB.Database.BeginTransaction())
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        UserInfo objUserInfo = _appDB.UserInfo.Where(p => p.Email == request.UserEmail && p.Userid != request.ID).SingleOrDefault();
                        if (objUserInfo != null)
                        {
                            throw new Exception(_languagePack["users_edit_message_exist_email"]);
                        }
                    ...
                    if (objData != null)
                    {
                        ...
                        objData.LoginType = request.UserLoginType;
                        _appDB.SaveChanges();
                        //删除原角色组
                        ...
                        Trans.Commit();
                        //添加日志
                        _baseService.UpdateLog<UserInfo>(objData, objData.Userid.ToString());
                        //返回信息
                        return new PostResponse()
                        {
                            Result = true,
        ...
        public PostResponse Delete(int[] ids)
        {
            ...
            try
            {
                ...
                _appDB.SaveChanges();
                //添加日志
                _baseService.DeleteLog("UserInfo", string.Join(",", ids));
                //返回信息
                return new PostResponse
                {
                    Result = true,
        ...
        public PostResponse Restore(int[] ids)
        {
            ...
            try
            {
                ...
                _appDB.SaveChanges();
                //添加日志
                _baseService.RestoreLog("UserInfo", string.Join(",", ids));
                //返回信息
                return new PostResponse
                {
                    Result = true,
        ...
        public PostResponse EditPassword(UsersPasswordEditRequest request)
        {
            ...
            try
            {
                ...
                if (objData != null)
                {
                    string _encryptPassword = _encryptService.EncryptPassword(request.Password, objData.PrivateKey);
                    //检查是否存在N次密码修改存在重复
                    List<string> objWebAppPasswordLogs = _logDB.WebAppPasswordLog.Where(p => p.UserID == objData.Userid).OrderByDescending(p => p.LogID).Select(p => p.Password).Take(_baseService.CurrentApplicationConfig.PasswordConfig.PastNum).ToList();
