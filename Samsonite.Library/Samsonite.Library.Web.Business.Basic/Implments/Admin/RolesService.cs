...
namespace Samsonite.Library.Web.Business.Basic
{
    public class RolesService : IRolesService
    {
        ...
        public RolesService(IBaseService baseService, appEntities appEntities)
        {
        ...
        public PostResponse Add(RolesAddRequest request)
        {
            ...
            using (var Trans = _appDB.Database.BeginTransaction())
            {
                try
                {
                    ...
                    Trans.Commit();
                    //添加日志
                    _baseService.InsertLog<SysRole>(objData, objData.Roleid.ToString());
                    //返回信息
            ...
            using (var Trans = _appDB.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (objData != null)
                    {
                        ...
                        Trans.Commit();
                        //添加日志
                        _baseService.UpdateLog<SysRole>(objData, objData.Roleid.ToString());
                        //返回信息
                        return new PostResponse()
                        {
                            Result = true,
        ...
        public PostResponse Delete(int[] ids)
        {
            ...
            using (var Trans = _appDB.Database.BeginTransaction())
            {
                try
                {
                    ...
                    Trans.Commit();
                    //添加日志
                    _baseService.DeleteLog("SysRole,SysRoleFunction", string.Join(",", ids));
                    //返回信息
                    return new PostResponse
                    {
                        Result = true,
