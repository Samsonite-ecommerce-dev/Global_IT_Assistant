...
namespace Samsonite.Library.Web.Business.Basic
{
    public class FtpConfigService : IFtpConfigService
    {
        ...
        public FtpConfigService(IBaseService baseService, appEntities appEntities)
        {
        ...
        public PostResponse Add(FtpConfigAddRequest request)
        {
            try
            {
                ...
                _appDB.SaveChanges();
                //添加日志
                _baseService.InsertLog<FTPInfo>(objData, objData.ID.ToString());
                //返回信息
        ...
        public PostResponse Edit(FtpConfigEditRequest request)
        {
            try
            {
                ...
                if (objData != null)
                {
                    ...
                    _appDB.SaveChanges();
                    //添加日志
                    _baseService.UpdateLog<FTPInfo>(objData, objData.ID.ToString());
                    //返回信息
                    return new PostResponse()
                    {
                        Result = true,
