...
namespace Samsonite.Library.Web.Business.Basic
{
    public class FunctionGroupService : IFunctionGroupService
    {
        ...
        public FunctionGroupService(IBaseService baseService, appEntities appEntities)
        {
        ...
        public PostResponse Add(FunctionGroupAddRequest request)
        {
            try
            {
                ...
                _appDB.SaveChanges();
                //添加日志
                _baseService.InsertLog<SysFunctionGroup>(objData, objData.Groupid.ToString());
                //返回信息
        ...
        public PostResponse Edit(FunctionGroupEditRequest request)
        {
            try
            {
                ...
                if (objData != null)
                {
                    ...
                    _appDB.SaveChanges();
                    //添加日志
                    _baseService.UpdateLog<SysFunctionGroup>(objData, objData.Groupid.ToString());
                    //返回信息
                    return new PostResponse()
                    {
                        Result = true,
        ...
        public PostResponse Delete(int[] ids)
        {
            using (var Trans = _appDB.Database.BeginTransaction())
            {
                try
                {
                    ...
                    Trans.Commit();
                    //添加日志
                    _baseService.DeleteLog("SysFunctionGroup,SysFunction", string.Join(",", ids));
                    //返回信息
                    return new PostResponse
                    {
                        Result = true,
