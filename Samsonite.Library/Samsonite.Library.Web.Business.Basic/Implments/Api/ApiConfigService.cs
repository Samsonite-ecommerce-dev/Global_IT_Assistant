...
namespace Samsonite.Library.Web.Business.Basic
{
    public class ApiConfigService : IApiConfigService
    {
        ...
        public ApiConfigService(IBaseService baseService, appEntities appEntities)
        {
        ...
        public PostResponse Add(ApiConfigAddRequest request)
        {
            using (var Trans = _appDB.Database.BeginTransaction())
            {
                try
                {
                    ...
                    Trans.Commit();
                    //添加日志
                    _baseService.InsertLog<WebApiAccount>(objData, objData.ID.ToString());
                    //返回信息
            ...
            using (var Trans = _appDB.Database.BeginTransaction())
            {
                try
                {
                    ...
                    if (objData != null)
                    {
                        ...
                        Trans.Commit();
                        //添加日志
                        _baseService.UpdateLog<WebApiAccount>(objData, objData.ID.ToString());
                        //返回信息
                        return new PostResponse()
                        {
                            Result = true,
