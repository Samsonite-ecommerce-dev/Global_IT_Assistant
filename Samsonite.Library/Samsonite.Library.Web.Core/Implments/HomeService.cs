...
namespace Samsonite.Library.Web.Core
{
    public class HomeService : IHomeService
    {
        ...
        public HomeService(IBaseService baseService, IEncryptService encryptService, appEntities appEntities, logEntities logEntities)
        {
        ...
        public PostResponse ResetLanguage(int id)
        {
            ...
            {
                //是否存在该语言
                LanguageType objAppLanguagePackModel = _baseService.LanguageTypeOption().Where(p => p.ID == id).SingleOrDefault();
                if (objAppLanguagePackModel != null)
                {
                    //设置语言包
                    _baseService.SaveCurrentLanguagePack(id);
                    //返回信息
        ...
        public PostResponse ResetPassword(EditPasswordRequest request)
        {
            ...
            try
            {
                ...
                if (objData != null)
                {
                    if (objData.Pwd.ToLower() != _encryptService.EncryptPassword(request.OldPassword, objData.PrivateKey))
                    {
