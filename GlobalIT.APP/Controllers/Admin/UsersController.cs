...
namespace GlobalIT.APP.Controllers
{
    public class UsersController : BaseController
    {
        ...
        public UsersController(IBaseService baseService, IUsersService usersService, IRolesService rolesService, appEntities appEntities) : base(baseService)
        {
        ...
        public JsonResult Initialize_Info(string type, int id)
        {
            ...
            var _languageList = _baseService.LanguageTypeConfigOption().Select(p => new { label = p.LanguageName, value = p.ID }).ToList();
            var _userTypeList = _userHelper.UserTypeObject().ToList();
            var _userLoginTypeList = _userHelper.UserLoginTypeObject().ToList();
            ...
            if (type == "index")
            {
                ...
                return Json(new
                {
                    ...
                    //登入类型
                    userLoginTypeList = _userLoginTypeList,
                ...
                if (objUser != null)
                {
                    ...
                    return Json(new
                    {
                        ...
                        model = new
                        {
                            ...
                            userLoginType= objUser.LoginType,
            ...
            var _result = new
            {
                ...
                rows = from dy in _list.Items
                       select new
                       {
                           ...
                           s5 = _userHelper.GetUserLoginTypeDisplay(dy.LoginType),
                           s6 = dy.Remark,
                           s7 = (dy.LastLoginTime != null) ? dy.LastLoginTime.Value.ToString("yyyy-MM-dd HH:mm:ss") : "",
                           s8 = (dy.Status == (int)UserStatus.Locked) ? "<label class=\"el-icon-lock text-danger\"></label>" : "<label class=\"el-icon-check text-primary\"></label>",
                       }
            };
            return Json(_result);
        }
